...
def get_patch(image: np.ndarray, row: int, col: int, patch_size: int) -> np.ndarray:
    ...
    return image[
        row - half_patch : row + half_patch + 1, col - half_patch : col + half_patch + 1
    ]
...
def calculate_nlm_value(
    ...
    for k in row_range:
        for neighbor_col in col_range:
            ...
            neighbor_patch = get_patch(
                padded_image, k + pad_size, neighbor_col + pad_size, patch_size
            )
    ...
    nlm_value = (
        weighted_sum / normalizing_factor if normalizing_factor > 0 else image[row, col]
    )
    ...
    for pixel in range(pixels_to_process):
        ...
        if row < height and col < width:
            nlm_value, weight, similarity_map = calculate_nlm_value(
                row, col, image, patch_size, search_window, h, use_full_image
            )
